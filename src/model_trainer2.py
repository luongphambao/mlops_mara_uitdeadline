...
class ModelTrainer:
    ...
    def train_model(prob_config: ProblemConfig, model_params, add_captured_data=False,model_name="xgb"):
        ...
        if prob_config.phase_id=="phase-2" and prob_config.prob_id=="prob-1":
            ...
            index_80=int(len(X)*0.7)
            test_df=raw_df[index_80:]
            train_x,train_y=X[:index_80],y[:index_80]
            test_x,test_y=X[index_80:],y[index_80:]
            #train_x=pd.concat([train_x,incorrect_x],ignore_index=True)
            #train_y=pd.concat([train_y,incorrect_y],ignore_index=True)
            # print(test_x.shape)
            # print(test_y.shape)
            # #add incorrect data
            # train_x=incorrect_x
            # train_y=incorrect_y
            # #drop df correct in raw_df
            # print(raw_df.shape)
            # df_correct=raw_df[~raw_df.index.isin(df_incorrect.index)]
            # print(df_correct.shape)
            # #exit()
            # print(train_x.shape)
            # print(train_y.shape)
            # test_x=df_correct.drop(columns=["label"])
            # test_y=df_correct["label"]
            #print(x_add.shape
            ...
            #exit()
            if add_captured_data:
                df_capture=pd.read_csv(os.path.join("data","result_voting.csv"))
                #shuffe data
        ...
        # test_df1.to_csv("data/predict_incorrect.csv",index=False)
        # test_df2=test_df[test_df["predict"]==test_df["label"]]
        # test_df2.to_csv("data/predict_correct.csv",index=False)
        #get confidence
...
df=df.rename(columns={"label_model":"label"})
#drop prob column
